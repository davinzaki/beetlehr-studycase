...
const openForm = () => {
    if (props.updateAction) {
        ...
        description.value =
            props.data.description === "null" || props.data.description === null
                ? ""
...
const handleDate = () => {
    if (form.value.start_date) {
        ...
        form.value.start = dayjs(form.value.start_date).format(
            "YYYY-MM-DD HH:mm"
        );
    }
...
const handleDownloadFile = async () => {
    ...
    axios
        .get(route("leave.downloadfile", { id: props.data.file_id }), {
            responseType: "blob",
        })
        .then((res) => {
            ...
            link.setAttribute(
                "download",
                "notice_board_file_" + timestamp + "." + props.data.file_ext
            );
...
const update = async (fd) => {
    ...
    axios
        .post(route("notice-board.update", { id: props.data.id }), fd)
        .then((res) => {
            emit("close");
            emit("successSubmit");
            form.value = ref({});
...
const create = async (fd) => {
    isLoading.value = true;
    axios
        .post(route("notice-board.create"), fd)
        .then((res) => {
            emit("close");
            emit("successSubmit");
            form.value = ref({});
...
<template>
    <VDialog
        ...
        :title="updateAction ? 'Update Notice Board' : 'Create Notice Board'"
        @opened="openForm"
        @closed="closeForm"
        size="xl"
    >
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <Datepicker
                        ...
                        position="left"
                        :clearable="false"
                        format="dd MMMM yyyy - HH:ii"
                        previewFormat="dd MMMM yyyy - HH:ii"
                ...
                <div class="col-span-2" v-if="updateAction">
                    <div v-if="form.type === 'document'">
                        ...
                        <VButton
                            ...
                            class="mt-auto"
                            :is-loading="downloadFileLoading"
                            v-if="data.file_id"
                        />
                    </div>
                </div>
